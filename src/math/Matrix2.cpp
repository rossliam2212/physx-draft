...
namespace physx::math {
     ...
     */
    Matrix2::Matrix2(float element00, float element01, float element10, float element11) {
    ...
    Vec2f Matrix2::operator*(const Vec2f& vec) const {
        float x{elements[0][0] * vec.getX() + elements[0][1] * vec.getY()};
        float y{elements[1][0] * vec.getX() + elements[1][1] * vec.getY()};
        return {x, y};
    }
